  ...
  return (
    <nav className='flex items-center py-6 px-16 justify-between gap-20 custom-nav'>
      ...
      <div className='flex items-center gap-6'>
        {/* 로고 */}
        <a className='text-3xl text-black font-bold gap-8' href='/'>E-commerce</a>
      </div>
        ...
        {variant === "auth" &&
          <ul className='flex gap-8'>
            <li className='text-black border border-black hover:bg-slate-100 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 focus:outline-none'>
              <NavLink to={"/v1/login"} className={({isActive})=> isActive ? 'active-link':''}>로그인</NavLink>
            </li>
            <li className='text-black border border-black hover:bg-slate-100 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 focus:outline-none'>
              <NavLink to="/v1/register" className={({isActive})=> isActive ? 'active-link':''}>가입하기</NavLink>
            </li>
          </ul>
        }
      </div>
