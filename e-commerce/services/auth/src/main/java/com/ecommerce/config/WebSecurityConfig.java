...
public class WebSecurityConfig {
	  ...
	  public WebSecurityConfig(UserDetailsService userDetailsService,JWTTokenHelper jwtTokenHelper,RESTAuthenticationEntryPoint authenticationEntryPoint) {
	  ...
	  private static final String[] PUBLIC_APIS = {
	        ...
	        "/swagger-ui/**",
	        "/swagger-ui.html",
	        "/oauth2/success"	// OAuth2 로그인 성공 후 리다이렉트 URL
	  };
	
	  @Bean
	  public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {	
		http
		    .csrf(AbstractHttpConfigurer::disable) // CSRF 보호 비활성화 -> JWT 기반이므로 세션 사용 안 함
		    .sessionManagement(session ->	// 세션 사용하지 않도록 설정
		            session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
		    .exceptionHandling(exception ->	// 인증 실패 시 실행할 핸들러 지정
